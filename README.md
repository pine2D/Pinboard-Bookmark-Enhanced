...
- **Quick links & status** — one-tap to your Unread, Network, Notes, and Popular pages; the toolbar icon shows at a glance whether the current page is unsaved, saved, or saved as *read later*
- **Page-to-Markdown** — turn the current page into clean Markdown with a built-in preview (rendered/raw views, table of contents, reading stats); copy or download as `.md`, styled `.html`, or a single-article `.epub` (Kindle/Kobo/KOReader), tune frontmatter (including author, publish date, site, cover image, and word count), image handling (keep, remove, or embed offline), and TOC, or send it straight to [Obsidian](https://obsidian.md), a GitHub Gist, or any webhook (Readwise-compatible). Site-aware extraction keeps Q&A, social posts, and forum threads readable (Zhihu, Hacker News, Stack Overflow, X/Twitter, …); pick [defuddle](https://github.com/kepano/defuddle) + [Turndown](https://github.com/mixmark-io/turndown) (local) or [Jina Reader](https://jina.ai/reader) (cloud). Images blocked by a site's hotlink protection are detected in the preview and can be restored with one click. Exports warn when kept image links are already known to fail — the Embed option packs what it can into downloaded files and reports the rest.
...
- **Reader tools** — highlight in five colors with notes that survive re-renders, translation, and page content changes (moved text is re-found automatically; unlocatable notes are kept and flagged, never lost), browse highlights in a Notebook panel, search the article (`/`, optional regex — your notes match too), peek footnotes in place, pick up where you left off, tune font size and line spacing in the Aa panel, set your reading width (680/880/1080 px), go distraction-free with a focus mode, or add an opt-in AI key-points skim (off by default); press `?` for the full shortcut list
