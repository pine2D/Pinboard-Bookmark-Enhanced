...
- **リーダーツール** — 5色でハイライトでき、メモは再描画・翻訳、さらにページ内容の変化をまたいでも保持されます（本文が移動しても自動で再配置し、見つからない場合もメモは失われず、その旨が明示されます）。ハイライトはノートブックパネルで一覧表示し、記事内を検索でき（`/`、正規表現にも対応 — メモも検索対象です）、脚注をその場でのぞき見表示し、続きから再開でき、読書幅を選べ（680/880/1080 px）、集中モードで気を散らさず読み進められ、オプトインの AI 要点スキム（既定でオフ）も追加できます。`?` キーでショートカット一覧を表示
