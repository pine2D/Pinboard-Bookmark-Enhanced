...
- **クイック保存・一括保存** — ポップアップを開かず、キーボードショートカットからページを保存（または*あとで読む*として保存）。開いているすべてのタブをまとめて保存することもでき、タブごとの AI タグ付け、リアルタイムの進捗表示、ひとつのタブセットへのまとめ（こちらも専用のキーボードショートカットに割り当て可能）にも対応。オフライン時の下書きはローカルキューに保持され、再接続後に Pinboard への保存が自動で再試行されます
...
- **クイックリンク・ステータス** — 未読 / ネットワーク / メモ / 人気の各ページへワンタップ。ツールバーアイコンを見れば、現在のページが未保存か、保存済みか、*あとで読む*として保存済みかがひと目でわかります
- **ページを Markdown に** — 現在のページをクリーンな Markdown に変換し、プレビューを内蔵（レンダリング/ソース表示の切り替え、目次、閲覧統計）。`.md`、スタイル付き `.html`、または単一記事の `.epub`（Kindle / Kobo / KOReader 対応）としてコピー・ダウンロードでき、frontmatter（著者、公開日、サイト、カバー画像、文字数を含む）・画像の扱い（保持・削除・オフライン埋め込み）・目次を調整したり、そのまま [Obsidian](https://obsidian.md)・GitHub Gist・任意の Webhook（Readwise 互換）へ送ることも可能。サイトに合わせた抽出で、Q&A、ソーシャル投稿、フォーラムのスレッドも読みやすく整形されます（知乎、Hacker News、Stack Overflow、X/Twitter、…）。抽出エンジンは [defuddle](https://github.com/kepano/defuddle) + [Turndown](https://github.com/mixmark-io/turndown)（ローカル）と [Jina Reader](https://jina.ai/reader)（クラウド）から選べます。サイトの直リンク対策でブロックされた画像はプレビューが検知し、ワンクリックで復元できます。エクスポート時、保持した画像リンクが読み込めないと分かっている場合は警告し、「埋め込み」は可能な範囲でダウンロードファイルに同梱し、できなかった分は別途知らせます。
...
- **リーダーツール** — 5色でハイライトでき、メモは再描画・翻訳、さらにページ内容の変化をまたいでも保持されます（本文が移動しても自動で再配置し、見つからない場合もメモは失われず、その旨が明示されます）。ハイライトはノートブックパネルで一覧表示し、記事内を検索でき（`/`、正規表現にも対応 — メモも検索対象です）、脚注をその場でのぞき見表示し、続きから再開でき、Aa パネルで文字サイズと行間を調整し、読書幅（680/880/1080 px）を選べ、集中モードで気を散らさず読み進められ、オプトインの AI 要点スキム（既定でオフ）も追加できます。`?` キーでショートカット一覧を表示
...
- **設定のバックアップ** — すべての設定（および任意でハイライトも）をファイルとしてエクスポート・インポートしたり、自分の WebDAV サーバー（Nextcloud など）へ自動でバックアップしたりできるので、設定やカスタムテーマをデバイス間で持ち運べます
