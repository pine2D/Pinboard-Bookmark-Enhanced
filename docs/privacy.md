...
| `optional_host_permissions: *://*/*` (declaration ceiling) | Allows Chrome to offer exact runtime grants for arbitrary user-selected origins. The extension requests only the current AI/Jina/Wayback/Gist/Webhook/WebDAV origin, the selected Batch tab origins, or — when you download a Markdown/HTML/EPUB export with the Embed (offline) image policy — the exact origins of the images referenced in that document (one-time prompt, can be declined); it never requests this wildcard itself |
