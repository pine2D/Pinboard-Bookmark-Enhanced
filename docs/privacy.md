...
| Authentication information | The extension stores and uses credentials you provide, such as your Pinboard API token, AI/Jina provider keys, Wayback S3 credentials, GitHub token, webhook authorization value or capability URL, WebDAV password, AnkiConnect API key, and Eudic OpenAPI authorization value |
...
15. **AnkiConnect** (`http://127.0.0.1:8765`, local loopback, optional; inactive until you click Send to Anki): the **Send to Anki** action sends each vocabulary book entry's term, reading, definition, contexts, source, and license text to a locally running Anki instance through the AnkiConnect add-on, authenticated with an API key you provide if AnkiConnect requires one. The request targets a fixed loopback address; it never reaches the public internet or the developer. The action requests only that exact origin and runs only when you click Send to Anki.

16. **Eudic OpenAPI** (`api.frdic.com`, optional; inactive until you click Send to Eudic): the **Send to Eudic** action sends the term text and language code of each vocabulary book entry in a supported language (English, French, German, or Spanish) to your Eudic account's default study list for that language, authenticated with an OpenAPI authorization value you provide. Entries in unsupported languages are skipped and not sent. The action requests only that exact origin and runs only when you click Send to Eudic.
...
| `optional_host_permissions: *://*/*` (declaration ceiling) | Allows Chrome to offer exact runtime grants for arbitrary user-selected origins. The extension requests only the current AI/Jina/Wayback/Gist/Webhook/WebDAV origin, the selected Batch tab origins, the exact origins of the images referenced in a Markdown/HTML/EPUB export when you download it with the Embed (offline) image policy, the exact origins of the failed images when you click **Fix** on the preview's blocked-images notice, the exact `https://freedictionaryapi.com` origin requested the first time you use the Dictionary lookup in Markdown preview, the exact `http://127.0.0.1:8765` AnkiConnect origin requested the first time you click **Send to Anki**, or the exact `https://api.frdic.com` Eudic origin requested the first time you click **Send to Eudic** (each a one-time prompt, can be declined; declining the Dictionary lookup grant only disables the Wiktionary lookup, and the AI explanation (if configured) and pronunciation still work; declining the AnkiConnect grant only disables Send to Anki, and TSV export still works; declining the Eudic grant only disables Send to Eudic, and TSV export still works); it never requests this wildcard itself |

Optional grants are requested from a direct user action and remain under Chrome's permission controls. Automatic feature paths that require an optional grant use `permissions.contains` only and never prompt. On upgrade from a legacy version that may have retained an all-sites grant, the extension performs a one-time removal of that wildcard grant. This can also clear matching old exact grants, but configurations are preserved and the next direct use can restore only the exact origin needed. On Chrome 142 and later, granting the AnkiConnect loopback origin can also trigger a separate Local Network Access (LNA) prompt from Chrome itself before the request reaches `127.0.0.1`; declining that prompt leaves Send to Anki unavailable without affecting any other permission.
...
- **Eudic OpenAPI** ([API docs](https://my.eudic.net/OpenAPI/Doc_Index)): your Eudic account's default study list receives the term text and language code of vocabulary book entries in a supported language only through an explicit Send to Eudic action, authenticated with an OpenAPI authorization value you provide
