...
├── options-vocab.js             # 生词 tab（列表/删除/TSV 导出/Anki·欧路发送链/词典包管理绑定）
...
├── md-dict.js                   # 词典视图（freedictionaryapi 词典槽 + AI 语境义槽 + speechSynthesis + pbp-vocab 生词 IDB）
├── md-vocab-echo.js             # 生词再现提示（CSS Custom Highlight 点状下划线 + 点击开词典；opt-in 默认关）
├── dict-pack.js                 # CC-CEDICT 离线词典包（文件导入 + IDB + zh 查词短路；options 静载，md-preview 懒加载）
