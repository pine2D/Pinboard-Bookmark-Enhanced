...
- **网络端点与 host 权限不变量**：required host 仅 Pinboard；AI / Jina / Wayback / Gist / Webhook / WebDAV / Free Dictionary API / Eudic、AnkiConnect 精确回环 origin 与 Batch 所选站点，只能从对应的直接用户动作请求当前精确 origin。后台/自动路径只做 `permissions.contains`，禁止运行时申请 wildcard。可配置网络端点必须 HTTPS，HTTP 仅允许字面 `localhost` / `127.0.0.1` / `[::1]`；LAN/public HTTP、凭据 URL 与无权限请求一律阻断并保留配置。升级时一次性清理 legacy all-sites grant。
- **WebDAV 旧布局迁移临时代码（引入 2026-07-23；最早清理 2026-08-23）**：清理前核查新版布局采用率和仍处于迁移状态的用户反馈。届时一并删除旧配置识别、迁移状态、迁移 UI、迁移文案及迁移专用测试；保留正常运行仍依赖的目标解析器、定位记录、路径冲突处理、collection 能力探测和路径校验。不得按日期自动删除，必须另行验证并提交。
